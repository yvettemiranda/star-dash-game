...
const state = {
  ...
  targetX: 360,
  targetY: 960,
  ship: { x: 360, y: 960, r: 23, shield: 0, armor: 3 },
  weapon: "normal",
  weaponTimer: 0,
  rocks: [],
  powerups: [],
...
function togglePause() {
  if (state.mode === "playing") {
    ...
    showPanel("暂停中", "火力核心会从天上掉下来，Boss 出现时集中躲弹。", "继续");
...
function gameOver() {
  ...
  showPanel("撞毁了", `本局 ${state.score} 分。下次优先捡护盾或散射核心。`, "再来");
}
...
function update(dt) {
  ...
  state.time += dt;
  state.speed = 290 + Math.min(380, state.time * 8);
  state.score = Math.floor(state.time * 10) + state.bonusScore;
...
function updateSpawns(dt) {
  ...
  if (state.spawnTimer <= 0) {
    spawnRock();
    state.spawnTimer = Math.max(0.5, 1.08 - state.time * 0.007);
  }

  state.powerTimer -= dt;
  if (state.powerTimer <= 0) {
    spawnPowerup();
    state.powerTimer = rand(5.2, 7.6);
  }
...
function firePlayer(dt) {
  ...
  if (state.weapon === "spread") {
    addBullet(-0.28, "#ffe06f", 11);
    addBullet(0, "#fff7bb", 12);
    addBullet(0.28, "#ffe06f", 11);
    state.shotTimer = 0.18;
  } else if (state.weapon === "beam") {
    addBullet(0, "#53acff", 18, 1.35);
    addBullet(0, "#bfe6ff", 10, 1.15, -13);
    addBullet(0, "#bfe6ff", 10, 1.15, 13);
    state.shotTimer = 0.11;
  } else {
    addBullet(0, "#36d6b5", 12);
    state.shotTimer = 0.23;
  }
}
...
function updateBoss(dt) {
  ...
  if (boss.fireTimer <= 0) {
    fireBoss(boss);
    boss.fireTimer = Math.max(0.42, 0.82 - state.time * 0.004);
  }
}
...
function spawnBoss() {
  const maxHp = 220 + state.time * 5;
  state.boss = {
    x: state.width / 2,
    y: -90,
    targetY: Math.max(122, state.height * 0.18),
    r: Math.min(62, state.width * 0.15),
    hp: maxHp,
    maxHp,
    t: 0,
    fireTimer: 1
...
function checkPlayerHits() {
  for (const rock of state.rocks) {
    if (distance(state.ship, rock) < state.ship.r + rock.r * 0.76) {
  ...
  for (const shot of state.enemyShots) {
    if (distance(state.ship, shot) < state.ship.r + shot.r) {
      absorbOrDie(shot.x, shot.y);
      return;
    }
  }

  if (state.boss && distance(state.ship, state.boss) < state.ship.r + state.boss.r * 0.82) {
...
function checkPowerups() {
  for (let i = state.powerups.length - 1; i >= 0; i -= 1) {
    ...
    if (distance(state.ship, item) < state.ship.r + item.r + 4) {
      state.bonusScore += 35;
      if (item.type === "shield") {
        state.ship.shield = 1;
      } else {
        state.weapon = item.type;
        state.weaponTimer = 12;
      }
      updateWeaponHud();
...
function absorbOrDie(x, y) {
  if (state.ship.shield > 0) {
    state.ship.shield = 0;
    updateWeaponHud();
    state.enemyShots = state.enemyShots.filter((shot) => distance(shot, state.ship) > 92);
    state.rocks = state.rocks.filter((rock) => distance(rock, state.ship) > 86);
    state.shake = 9;
    burst(x, y, "#8ef36e", 30);
    return;
  }
